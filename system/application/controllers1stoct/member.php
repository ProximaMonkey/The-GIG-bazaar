	...
	function email_link($email)
		{
			...
			function requestpayout()
			{
				...
				if ($this->form_validation->run() == FALSE)
				{
					...
					$current_balance = $this->Member_model->balance($this->session->userdata('id'));
					if($amount > $current_balance)
					{
					$this->session->set_flashdata('payout', '<div class="form-error"><p>Amount you have entered is greater than your balance.</p></div>');
					redirect('member/payment');
					end;
					}
					...
					if($queue['count'] > 0)
					{
						$this->session->set_flashdata('payout', '<div id="highlight"><p>There is a payment request you made on '.$queue['date'].' already in queue</p></div>');
						redirect('member/payment');
						end;
					}
					$date = date('Y-m-d');
					$data = array('amount'=>$this->input->post('request_amount'),'from'=>$this->session->userdata('id'),'request_date'=>$date);
					//$this->db->where('id',$this->session->userdata('id'));
					$this->db->insert('withdraw',$data);
					$this->session->set_flashdata('payout', '<div class="form-success"><p>Request has been sent.We will send an email once approved.</p></div>');
					redirect('member/payment');
				}
