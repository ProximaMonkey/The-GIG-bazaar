    ...
    function Main_model()
    {
		...
		function add_member()
		{
			...
			//Now send the email verification link
				...
				$this->email->from('info@asda.com', 'asd.com');
				$this->email->to($this->input->post('email'));
				$this->email->subject('Verify Email address: dummy.com');
				...
				<br>ASD Team<br><br><b>If you have any queries regarding the email, please email us at <a href=mailto:support@asd.com>support@asd.com</a>');
				$this->email->send();
		...
		function gig_listing_homepage($num,$offset,$sort)
		{
				...
				foreach($query->result() as $row)
				{
					...
					<div id='blog-image'><img src='".base_url()."/assets/timthumb.php?src=".base_url()."/images/gigs/".$row->image."&h=56&w=56&zc=1 alt=''></div><!--blog image ends-->
					
					<div id='tag'>
					<div class='heading-of-gig'><b><a href='".site_url('gig/single/'.$row->id)."' class=listing-title>$row->title</a></b></div><!--heading of bg ends--><div class='tag-bg-left'>Tagged as</div>
					";
					...
					<div class='by'>By <a href='".site_url('user/view/'.$this->Common_model->getusername($row->member_posted_by))."'>".$this->Common_model->convertname($row->member_posted_by)."</a> on ".$this->Common_model->convert_date($row->date_added)."</div><!--by ends-->
					<div class='order'>
