    ...
    function Member_model()
    {
		...
		function item_selling($id)
		{
			...
			foreach($query->result() as $row)
			{
				if($row->member_disabled == 'y')
				{
					...
					<br/></div><div id=single_listing_content><a href=".site_url('service/single/'.$row->id)." class=listing-title>".$row->title."</a>".
						substr(strip_tags($row->description),0,100)."...</div>						
							
							<div id=meta>Posted on ".$this->Common_model->convert_date($row->date_added)." - View: - $row->views - <a href='".site_url('service/edit/'.$row->id)."'>Edit</a> | <a href='".site_url('service/delete/'.$row->id)."'>Delete</a> |";
								if($row->member_disabled == 'y')
								{
									$selling .= "<a href='".site_url('service/enable/'.$row->id)."'>Enable</a></div>";
								}
								else {
									$selling .= "<a href='".site_url('service/disable/'.$row->id)."'>Disable</a></div>";
								}
		...
		function gigs_foryou($id)
		{
			...
			foreach($query->result() as $gigs)
			{
				...
				{
					$status = "<font color='red'>Dispute Raised</font>";
				}
				
					$foryou .= '<div class=single_order>
									<div class=order_date>
						Date:<br/> '.$gigs->order_date.'</div>
						<div class=order_title><a href='.site_url('service/single/'.$gigs->orderid).' class=listing-title>'.$title.' </a></div>
						<div class=order_status>'.$status.'</div>
						<div class="order_actions">';
						if($order_status == '1')
						{
							$foryou .= "<b style='color:green'>Gig Accepted on: </b>".$gigs->order_accept_date." | <a href='".site_url('service/completed/'.$gigs->orderid)."'>Gig Completed</a>";
						}
						if($order_status == '0')
						{
						...
						if($order_status == ""){
							$foryou .= '<a href="'.site_url('service/acceptorder/'.$gigs->orderid).'">Accept this order</a> | <a href="'.site_url('service/rejectorder/'.$gigs->orderid).'">Reject this order</a>';
						}
						$foryou .= ' | Send a message to <a href="'.site_url('member/sendmessage/'.$this->Common_model->getusername($gigs->order_for)).'">'.$this->Common_model->convertname($gigs->order_for).'</a></div>	
		...
		function gigs_ordered($id)
		{
			...
			foreach($query->result() as $gigs)
			{
					...
					$ordered .= '<div class=single_order>
									<div class=order_date>
						Date:<br/> '.$gigs->order_date.'</div>
						<div class=order_title><a href='.site_url('service/single/'.$gigs->gigid).">".$title.' </a></div>
						<div class=order_status><b>Current Status:</b> '.$status.'</div>';
						if($gigs->order_accepted != '2')
						{
							$ordered .= '<div class="order_actions">	<a href="'.site_url('service/contactseller/'.$gigs->orderid).'">Contact Seller</a> | <a href="'.site_url('service/cancelorder/'.$gigs->orderid).'">Cancel order</a></div>';
						}
						else {
							...
							$days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
								if($days <= 15)
								{
							$ordered .= '<div class="order_actions">	<a href="'.site_url('service/contactseller/'.$gigs->orderid).'">Contact Seller</a> | <a href="'.site_url('service/dispute/'.$gigs->orderid).'">File Dispute</a></div>';
							
								}
							}
							else {
									$ordered .= '<div class="order_actions">	<a href="'.site_url('service/contactseller/'.$gigs->orderid).'">Contact Seller</a> | <font color=red>Dispute Raised</font> </div>';
							}
		...
		function withdraw()
		{
			$amount = "";
				...
				if($count <= "0")
				{
					...
					$withdraw['total_amount'] = "<b id='highlight'>".CURRENCY.$amount."</b>";
					$withdraw['disabled'] = "0";
					if($amount < '25')
					{
		...
		function balance($id)
		{
			$amount = "";
			$this->db->where('to', $id);
			$this->db->where('status','0'); // Get all the unpaid payments only
			$query = $this->db->get('payments');
				foreach($query->result() as $row)
				{
					$amount .= $amount + $row->amount;
		...
		function get_inbox()
		{
			...
			foreach($query->result() as $row)
			{
				...
				{
					$inbox .= "<img src='".base_url()."/images/message.png' align='absmiddle' class='icon'><b><a href='".site_url('member/readmessage/'.$row->id)."'>".substr($row->message,0,30)."</a></b>";
				}
				else {
					$inbox .= "<img src='".base_url()."/images/message_read.png' align='absmiddle' class='icon'><a href='".site_url('member/readmessage/'.$row->id)."'>".substr($row->message,0,30)."</a>";
				}
