			...
			foreach($query->result() as $gigs)
			{
				...
				if($gigs->order_dispute == '1')
				{
					...
					$foryou .= '<div class=single_order>
						...
						<div class=order_title><a href='.site_url('gig/single/'.$gigs->orderid).' class=listing-title>'.$title.' </a></div>
		...
		function gigs_ordered($id)
		{
			...
			foreach($query->result() as $gigs)
			{
					...
					$ordered .= '<div class=single_order>
						...
						if($gigs->order_accepted != '2')
						{
							$ordered .= '<div class="order_actions">	<a href="'.site_url('gig/contactseller/'.$gigs->orderid).'">Contact Seller</a> | <a href="'.site_url('gig/cancelorder/'.$gigs->orderid).'">Cancel order</a></div>';
						}
						else {
