    ...
    function Gig_model()
    {
		...
		function get_job_listing($id)
		{
			...
			function get_filtered_listing($filter,$num,$offset)
			{
					...
					$list .= "<div id='filter_title'><b>Filtered by:</b> <span>".$filter_title."</span></div>";
						...
						if($query->num_rows() <= 0)
						{
							$list .= "<p>There are no gigs in <b>".$filter_title."</b></p>";
						}
					return $list;
			}
		...
		function get_person_detail($id)
		{
			//Get the person id from the gig table
				...
				if($row1->member_name == "")
				{
					$data['name'] = $row1->username;
				}
				else {
						$data['name'] = $row1->member_name;
				}
				if($row1->member_facebook != "")
				{
				...
				{
					$data['description'] = "No information currently provided.";
				}
				else {
		...
		function search($searchstring,$num,$offset)
		{
			...
			function edit_details($id)
			{
					...
					$data['tag1'] = $tags[0];$data['tag2'] = $tags[1];$data['tag3'] = $tags[2];$data['tag4'] = $tags[3];$data['tag5']= $tags[4];
