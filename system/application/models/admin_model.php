		...
		function unapproved_job_count()
		{
			...
			function user_list()
			{
				...
				{	
					...
					{
						$approved = "<font color='red'>[not verified]</font>";
					}				
					else {
						$approved = "";
					}
					if($row->banned == 'y')
					{
						$banned = "<font color='red'>[banned]</font>";
					}				
					else {
						$banned = "";
					}
					
					$list .= "<div id='list_title'>$approved $banned <b>$row->member_username</b> - ".$email."</div>";
					$list .= "<div id='list_action'>";
				if($row->member_verified == 'n')
				{
					if($row->level != "-1")
					{
						...
						if($row->level != "-1")
						{
							$list .= "|  <a href='".site_url('admin/userban/'.$row->id)."' id='user_delete'>Ban</a>";
						}
							$list .= "</div></div>";
				}
			...
			function withdraw_request()
			{
				...
				$q = $this->db->get('withdraw',7);
				foreach($q->result() as $row)
				{
					$html .= "<li> - $row->amount from ".$this->Common_model->convertname($row->request_from)."</li>";
				}
			...
			function recentgigs()
			{
				...
				foreach($q->result() as $row)
				{
					$html .= "<li> - $row->title from ".$this->Common_model->convertname($row->member_posted_by)."</li>";
				}
