	...
	<li class="highlight"><a href="<?php echo site_url('member/payment');?>">Payment</a></li>
		...
		<?php echo $this->session->flashdata('payout'); ?>
				<form method="post" action="<?php echo site_url('member/requestpayout');?>" id="requestpayout">
					<input type='hidden' value='<?php echo $withdraw['amount'];?>' name='amount'>
					<input type='hidden' value='<?php echo $withdraw['qid'];?>' name='ids'>
									<ul>
										<li><label>Current Balance</label><div> <?php echo $withdraw['total_amount']?>
			...
			<li><div><input type="submit" value="withdraw" name="submit" id="submit" <?php echo $withdraw['submit']?>></div> </li>
							<?php
							}
